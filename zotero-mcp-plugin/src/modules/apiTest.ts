...
export async function testPDFExtraction(): Promise<string> {
  ...
  try {
    ...
    if (!regularItemIDs.length) {
      report +=
        "PDF Content Test: SKIPPED - No regular items found in the library.\n";
    ...
    ztoolkit.log(
      `[APITest] Checking ${regularItemIDs.length} items for PDF attachments...`,
    );
    ...
    if (!itemWithPDF) {
      report +=
        "PDF Content Test: SKIPPED - No item with a PDF attachment found among the checked items.\n";
    ...
    try {
      ...
      ztoolkit.log(
        `[APITest] All text extracted. Preview: ${allText.substring(0, 100)}...`,
      );
    } catch (e) {
