...
这是 [Zotero](https://www.zotero.org/) 的Zotero MCP Plugin。
...
- 开发工具参考
  ...
  - [📌 Zotero Zotero MCP Plugin](https://github.com/windingwind/zotero-plugin-template) (即本仓库)
...
- **分类管理 (Collections Management)**: 通过专用 API 浏览、搜索和检索分类中的条目。
- **高级标签搜索 (Advanced Tag Search)**: 使用强大的标签查询筛选条目，支持 `any`/`all`/`none` 模式和 `exact`/`contains`/`startsWith` 匹配方式。
- **PDF 文本提取 (PDF Text Extraction)**: 从 PDF 附件中提取文本内容，支持提取全文或指定页面。
...
Zotero MCP Plugin为 bootstrap 插件提供了一些新的 API. 我们有两个原因使用这些 API，而不是使用 `createElement/createElementNS`：

- 在 bootstrap 模式下，插件必须在推出（禁用或卸载）时清理所有 UI 元素，这非常麻烦。使用 `createElement`，Zotero MCP Plugin将维护这些元素。仅仅在退出时 `unregisterAll` .
