...
async function makeZoteroRequest(endpoint: string, params: Record<string, any> = {}, pathParams: Record<string, string> = {}) {
...
async function main() {
  ...
  server.tool(
    "search",
    "Search the Zotero library with basic and advanced criteria. Basic: keywords, title, item key, tags. Advanced: title operators (exact, contains, regex), date/year ranges, relevance scoring with field boosting, custom sorting. Returns core bibliographic data. Use this for finding literature items.",
    searchSchema,
    async (params: any) => {
      try {
  ...
  server.tool(
    "get_collections",
    "Gets a hierarchical list of all collections.",
    {},
    async () => {
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `Failed to get collections: ${error instanceof Error ? error.message : String(error)}` }],
        };
      }
    }
  );
  ...
  server.tool(
    ...
    async (params: { q: string }) => {
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `Collection search failed: ${error instanceof Error ? error.message : String(error)}` }],
        };
      }
    }
  );
  ...
  server.tool(
    ...
    getCollectionDetailsSchema,
    async (params: { collectionKey: string }) => {
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `Failed to get collection details: ${error instanceof Error ? error.message : String(error)}` }],
        };
      }
    }
  );
  ...
  server.tool(
    ...
    "Gets all items within a specific collection.",
    ...
    async (params: { collectionKey: string }) => {
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `Failed to get collection items: ${error instanceof Error ? error.message : String(error)}` }],
        };
      }
    }
  );
  ...
  server.tool(
    "get_pdf_content",
    "Extract text content from PDF attachments of a Zotero item",
    getPdfContentSchema,
    async (params: { itemKey: string; page?: number; format?: 'text' | 'json' }) => {
      try {
        const { itemKey, ...queryParams } = params;
        const result = await makeZoteroRequest(
          "/items/:itemKey/pdf-content",
          queryParams,
          { itemKey }
        );
        ...
        return {
          ...
          content: [{ type: "text" as const, text: isJson ? JSON.stringify(result, null, 2) : String(result) }],
        };
      } catch (error) {
        return {
          content: [{ type: "text" as const, text: `Failed to get PDF content: ${error instanceof Error ? error.message : String(error)}` }],
        };
      }
    }
  );
  ...
  server.tool(
    "search_annotations",
    "Search all notes, PDF annotations and highlights in the Zotero library. Returns metadata first (pagination, total count, search time) followed by results. Use preview mode (default) for overview, detailed mode for full content. Supports filtering, sorting and pagination.",
    ...
    async (params: any) => {
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `Annotation search failed: ${error instanceof Error ? error.message : String(error)}` }],
        };
      }
    }
  );
  ...
  server.tool(
    "get_item_notes",
    "Get notes content for a specific item with pagination support. Returns metadata first (total count, pagination info) followed by notes data.",
    getItemNotesSchema,
    async (params: { itemKey: string; limit?: string; offset?: string }) => {
      try {
        const { itemKey, ...queryParams } = params;
        const result = await makeZoteroRequest(
          "/items/:itemKey/notes",
        ...
        return {
          content: [{ type: "text" as const, text: `Failed to get item notes: ${error instanceof Error ? error.message : String(error)}` }],
        };
      }
    }
  );
  ...
  server.tool(
    ...
    "Get PDF annotations and highlights for a specific item with pagination support. Returns metadata first (total count, pagination) followed by annotations. Results are sorted by page position.",
    getItemAnnotationsSchema,
    async (params: { itemKey: string; type?: string; color?: string; limit?: string; offset?: string }) => {
      try {
        const { itemKey, ...queryParams } = params;
        const result = await makeZoteroRequest(
          "/items/:itemKey/annotations",
          queryParams,
          { itemKey }
        );
        return {
        ...
        return {
          content: [{ type: "text" as const, text: `Failed to get item annotations: ${error instanceof Error ? error.message : String(error)}` }],
        };
      }
    }
  );
  ...
  server.tool(
    ...
    "Get the complete content of a specific annotation by its ID. Returns full content without truncation.",
    getAnnotationByIdSchema,
    async (params: { annotationId: string }) => {
      try {
        const result = await makeZoteroRequest(
        ...
        return {
          content: [{ type: "text" as const, text: `Failed to get annotation: ${error instanceof Error ? error.message : String(error)}` }],
        };
      }
    }
  );
  ...
  server.tool(
    ...
    "Get complete content of multiple annotations by their IDs in a single request. Returns full content for all requested annotations.",
    getAnnotationsBatchSchema,
    async (params: { ids: string[] }) => {
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `Batch annotation request failed: ${error instanceof Error ? error.message : String(error)}` }],
        };
      }
    }
  );
